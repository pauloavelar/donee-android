...
public class WelcomeLogic {
    ...
    private WelcomeLogic(Context c) { this.context = c; }
    private void setContext(Context c) { this.context = c; }

    public static WelcomeLogic using(Context context) {
        if (instance == null) {
            instance = new WelcomeLogic(context);
        } else {
